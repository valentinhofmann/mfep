...
def main():
    ...
    # Write results to text files
    with open('results/scores_main_delta_r.txt', 'w') as f:
        f.write(' '.join(['{:3.3f}'.format(score) for score in scores]))
        f.write(' {:3.3f} $\\pm$ {:3.3f} '.format(np.mean(scores), np.std(scores)))
    with open('results/imps_main_delta_r.txt', 'w') as f:
        for i in range(len(imps[0])):
            f.write(' '.join(['{:3.3f}'.format(im[i]) for im in imps]))
            f.write(' {:3.3f} $\\pm$ {:3.3f} '.format(np.mean([im[i] for im in imps]), np.std([im[i] for im in imps])))
            f.write('\n')
